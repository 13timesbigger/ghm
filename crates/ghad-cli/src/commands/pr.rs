...
pub async fn handle_list(repo: Option<String>, org: Option<String>) -> Result<()> {
    ...
    let prs = match (repo, org) {
        (Some(r), Some(o)) => {
            ...
            ghad_core::github::pulls::list_pulls(&client, parts[0], parts[1])
                .await
        ...
        (None, Some(o)) => {
            ghad_core::github::pulls::list_pulls_by_org(&client, &o)
                .await
        ...
        (None, None) => {
            ...
            for r in repos.repositories {
                if r.watch_prs {
                    ...
                    if parts.len() == 2 {
                        if let Ok(p) = ghad_core::github::pulls::list_pulls(&client, parts[0], parts[1]).await {
                            all_prs.extend(p);
                        }
                    }
                }

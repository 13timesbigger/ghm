...
pub async fn handle_list(
    ...
    let config = ghad_core::config::load_default_config().context(
    ...
    let issues = match (repo, org) {
        (Some(r), Some(o)) => {
            ...
            ghad_core::github::issues::list_issues(&client, parts[0], parts[1])
                .await
        ...
        (None, Some(o)) => {
            ghad_core::github::issues::list_issues_by_org(&client, &o)
                .await
                .context(format!("Failed to fetch issues for org '{}'", o))?
        }
        (None, None) => {
            let config_dir = ghad_core::config::default_config_dir()?;
            let store = ghad_core::store::ObservedStore::new(&config_dir);
            ...
            for r in repos.repositories {
                if r.watch_issues {
                    ...
                    if parts.len() == 2 {
                        if let Ok(i) = ghad_core::github::issues::list_issues(&client, parts[0], parts[1]).await {
                            all_issues.extend(i);
                        }
                    }
                }

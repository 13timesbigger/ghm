...
const CONFIG_DIR: &str = "ghm";
/// Default configuration file name.
const CONFIG_FILE: &str = "config.json";

/// Returns the default configuration directory: `~/.config/ghm/`.
pub fn default_config_dir() -> Result<PathBuf> {
    dirs::config_dir()
        .map(|d| d.join(CONFIG_DIR))
        .ok_or_else(|| GhmError::ConfigInvalid {
            message: "could not determine config directory".into(),
        })
}

/// Returns the default config file path: `~/.config/ghm/config.json`.
...
pub fn load_config(path: &Path) -> Result<Config> {
    ...
    let contents = std::fs::read_to_string(path).map_err(|e| GhmError::ConfigRead { source: e })?;
    let config: Config =
        serde_json::from_str(&contents).map_err(|e| GhmError::ConfigParse { source: e })?;
    Ok(config)
}
...
pub fn save_config(path: &Path, config: &Config) -> Result<()> {
    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent).map_err(|e| GhmError::ConfigWrite { source: e })?;
    }
    let json =
        serde_json::to_string_pretty(config).map_err(|e| GhmError::ConfigParse { source: e })?;
    std::fs::write(path, json).map_err(|e| GhmError::ConfigWrite { source: e })?;
    Ok(())
}

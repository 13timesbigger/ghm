...
fn atomic_write(path: &Path, data: &[u8]) -> Result<()> {
    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent).map_err(|e| GhadError::StoreWrite { source: e })?;
    }
    let tmp_path = path.with_extension("tmp");
    std::fs::write(&tmp_path, data).map_err(|e| GhadError::StoreWrite { source: e })?;
    std::fs::rename(&tmp_path, path).map_err(|e| GhadError::AtomicRename {
        from: tmp_path,
        to: path.to_path_buf(),
        source: e,
    })?;

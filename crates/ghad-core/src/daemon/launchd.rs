...
/// Manages macOS launchd plist generation for the GHAD daemon.
#[derive(Debug, Clone)]
...
impl LaunchdManager {
    ...
    pub fn default_plist_path() -> Result<PathBuf> {
        let home = dirs::home_dir().ok_or_else(|| GhadError::LaunchdError {
...
"#,
    ...
    pub fn install_plist(&self, path: &Path) -> Result<()> {
        if let Some(parent) = path.parent() {
            std::fs::create_dir_all(parent).map_err(|e| GhadError::LaunchdError {
                message: format!("failed to create plist directory: {e}"),
            })?;
        }
        let content = self.generate_plist();
        std::fs::write(path, content).map_err(|e| GhadError::LaunchdError {
    ...
    pub fn uninstall_plist(&self, path: &Path) -> Result<()> {
        if path.exists() {
            std::fs::remove_file(path).map_err(|e| GhadError::LaunchdError {
...
mod tests {
    ...
    fn make_manager() -> LaunchdManager {
        LaunchdManager::new(
            ...
            PathBuf::from("/Users/test/.config/ghad"),
        )
    }
    ...
    fn generate_plist_contains_required_keys() {
        ...
        assert!(plist.contains("/Users/test/.config/ghad"));
    ...
    fn plist_with_spaces_in_paths() {
        let mgr = LaunchdManager::new(
            ...
            PathBuf::from("/Users/my user/.config/ghad"),
        );
        let plist = mgr.generate_plist();
        assert!(plist.contains("/usr/local/bin/my ghad"));
        assert!(plist.contains("/Users/my user/.config/ghad"));
    }
}

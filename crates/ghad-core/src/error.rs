...
/// Top-level error type for the ghad-core library.
#[derive(Debug, Error)]
pub enum GhadError {
    // ── Config errors ──────────────────────────────────────────────
...
pub type Result<T> = std::result::Result<T, GhadError>;
...
mod tests {
    ...
    fn config_not_found_display() {
        let err = GhadError::ConfigNotFound {
    ...
    fn config_invalid_display() {
        let err = GhadError::ConfigInvalid {
    ...
    fn store_not_found_display() {
        let err = GhadError::StoreNotFound {
    ...
    fn token_missing_display() {
        let err = GhadError::TokenMissing;
        assert!(err.to_string().contains("not configured"));
    }

    #[test]
    fn device_flow_display() {
        let err = GhadError::DeviceFlow {
    ...
    fn daemon_already_running_display() {
        let err = GhadError::DaemonAlreadyRunning { pid: 1234 };
        assert!(err.to_string().contains("1234"));
    }

    #[test]
    fn daemon_not_running_display() {
        let err = GhadError::DaemonNotRunning;
        assert!(err.to_string().contains("not running"));
    }

    #[test]
    fn agent_dispatch_display() {
        let err = GhadError::AgentDispatch {
            agent: "codex".into(),
    ...
    #[test]
    fn launchd_display() {
        let err = GhadError::LaunchdError {
    ...
    fn json_from() {
        ...
        let err: GhadError = json_err.into();
        assert!(!err.to_string().is_empty());
    }

    #[test]
    fn other_display() {
        let err = GhadError::Other("custom error".into());
        assert!(err.to_string().contains("custom error"));
    }

    #[test]
    fn config_read_display() {
        let err = GhadError::ConfigRead {
    ...
    fn config_parse_display() {
        ...
        let err = GhadError::ConfigParse { source: json_err };
        assert!(err.to_string().contains("parse config"));
    }

    #[test]
    fn store_read_display() {
        let err = GhadError::StoreRead {
    ...
    fn store_parse_display() {
        ...
        let err = GhadError::StoreParse { source: json_err };
        assert!(err.to_string().contains("parse store"));
    }

    #[test]
    fn atomic_rename_display() {
        let err = GhadError::AtomicRename {
    ...
    fn result_alias_works() {
        ...
        let err: Result<i32> = Err(GhadError::TokenMissing);
        assert!(err.is_err());
    }
    ...
    fn error_is_send_sync() {
        ...
        // GhadError should be Send + Sync for async usage
        assert_send_sync::<GhadError>();
    }
